...
- `main` — synced with Danny's upstream (`danny-avila/LibreChat`). **Never target this with PRs.** Only upstream syncs land here.
- `main-development` — our custom branch. All PRs target here.
- Feature branches: `feat/<name>`, `fix/<name>`

**Starting a new task — always worktree from origin/main-development:**

```bash
git -C ~/workspace/gptchina fetch origin
git -C ~/workspace/gptchina worktree add /tmp/<short-name> -b feat/<name> origin/main-development
cd /tmp/<short-name>
```

**Never:**
- `git checkout -b` inside an existing worktree (inherits wrong branch state)
- Branch from a local branch without fetching first
- Branch from another feature branch
- Open a PR targeting `main` — that branch is reserved for upstream syncs only
...
8. PR opened against `main-development` (never `main`)
